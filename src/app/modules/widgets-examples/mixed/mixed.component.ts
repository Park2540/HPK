...
// caltime(){
//   this.timess =((this.Levellis.hours*3600) + (this.Levellis.minutes*60 ))
//   this.Levellis.time=this.timess

// }
  onSubmit(){
    if (this.Levellisform.valid){
      this.time =((this.Levellis.hours*3600) + (this.Levellis.minutes*60 ))
    
      this.Levellis.time = this.time;
      this.SettingService.createNewlevel(this.Levellisform.value,this.time).subscribe((res:any) => {
  ...
  onSubmitUpdate(){
    ...
    if (this.updateLevellis.valid){
      ...
      this.Levellis.time = this.time;
      this.SettingService.updatelevel(this.Levellis.id,this.updateLevellis.value, this.time).subscribe((res:any) => {
